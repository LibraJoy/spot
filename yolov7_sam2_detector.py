...
class YOLOv7SAM2Detector:
    def __init__(self):
        rospy.loginfo("Initializing YOLOv7+SAM2 Detector...")
        ...
        # Publish to the same shared detection topic as YOLOv8
        self.yolo_detect_pub = rospy.Publisher('/yolo/detection', Detection2DArray, queue_size=10)
        
        # Individual publishers for debugging/visualization
        ...
        rospy.loginfo("YOLOv7+SAM2 Detector initialized successfully!")
    ...
    def _load_yolov7_model(self):
        ...
        try:
            ...
            model_paths = [
                "best.7.10.pt",
                "/root/spot_ws/src/spot/models/best.7.10.pt",
                "/home/cerlab/Downloads/best.7.10.pt"
            ]
            
            model_loaded = False
            for path in model_paths:
                try:
                    self.model = yolov7.load(path)
                    if self.device.type != "cpu":
                        self.model = self.model.to(self.device)
                    rospy.loginfo(f"YOLOv7 model loaded from: {path}")
                    model_loaded = True
                    break
                except Exception as e:
                    rospy.logdebug(f"Failed to load model from {path}: {e}")
                    continue
    ...
    def _publish_detection_results(self, ros_image_msg, boxes, confs, cats, names, masks, mask_base, img_overlay):
        ...
        try:
            ...
            for i, (box, conf, cat) in enumerate(zip(boxes, confs, cats)):
                ...
                # Add individual mask if available - FIX THE MASK HANDLING
                if masks is not None and i < len(masks):
                    try:
                        # Get the mask for this detection
                        mask = masks[i]
                        
                        # Debug: Print mask shape to understand the issue
                        rospy.logdebug(f"Original mask shape: {mask.shape}")
                        
                        # Handle different mask shapes properly
                        if mask.ndim == 3:
                            # If mask has 3 dimensions, take the first channel
                            if mask.shape[0] == 1:
                                mask_img = mask[0]  # Remove first dimension if it's 1
                            elif mask.shape[2] == 1:
                                mask_img = mask[:, :, 0]  # Remove last dimension if it's 1
                            else:
                                mask_img = mask  # Use as is if all dimensions > 1
                        elif mask.ndim == 2:
                            mask_img = mask  # Already 2D
                        else:
                        ...
                        rospy.logdebug(f"Processed mask shape: {mask_img.shape}")
                        ...
                        rospy.logdebug(f"Mask shape was: {mask.shape if masks is not None and i < len(masks) else 'N/A'}")
            ...
            rospy.loginfo(f"Published YOLOv7+SAM2 detection #{self.id} with {len(detection_array.detections)} detections to shared /yolo/detection topic")
    ...
    def _publish_empty_results(self, ros_image_msg):
        ...
        try:
            ...
            empty_mask = np.zeros((self.h_org, self.w_org), dtype=np.uint8)
            mask_msg = self.bridge.cv2_to_imgmsg(empty_mask, encoding="mono8")
            vis_msg = self.bridge.cv2_to_imgmsg(self.img_data, encoding="bgr8")
...
def main():
    rospy.init_node('yolov7_sam2_detector', anonymous=True)
    
    try:
        detector = YOLOv7SAM2Detector()
        rospy.loginfo("YOLOv7+SAM2 Detector node started successfully!")
        rospy.spin()
    except rospy.ROSInterruptException:
        rospy.loginfo("YOLOv7+SAM2 Detector node interrupted")
    except Exception as e:
        rospy.logerr(f"Failed to start YOLOv7+SAM2 Detector: {e}")
    finally:
        rospy.loginfo("YOLOv7+SAM2 Detector node shutting down")
