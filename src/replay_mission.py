...
def mission_demo(raw_args=None):
    ...
    if args.mission_type == 'simple':
        ...
        if args.autowalk_mission_file:
            ...
            mission_file = f'{map_directory}/missions/autogenerated'
            # Missions recorded on tablet after 3.2 are saved as "autogenerated.node".
            # If autogenerated.node exists prefer it.
            if os.path.isfile(f'{mission_file}.node'):
                mission_file = f'{mission_file}.node'

        print(
            f'[ REPLAYING AUTOWALK MISSION {mission_file} : MAP DIRECTORY {map_directory} : HOSTNAME {args.hostname} ]'
        )
    ...
    if not os.path.isfile(mission_file):
        ...
        if args.mission_type == 'autowalk' and not args.autowalk_mission_file:
            robot.logger.fatal(
                f'Unable to find mission file: {mission_file} or {mission_file}.node.')
        else:
...
def init_clients(robot, mission_file, map_directory, do_map_load, disable_alternate_route_finding,
    ...
    if do_map_load:
        if not os.path.isdir(map_directory):
            robot.logger.fatal(f'Unable to find map directory: {map_directory}.')
            sys.exit(1)
...
if __name__ == '__main__':
    args = '192.168.80.3', 'autowalk', './mission3'
    mission_demo(args)
