...
def path_callback(path_msg):
    ...
    # print(path_msg.poses.pose.position)
    # Extracts waypoints from the path message and commands the robot to follow them.


    for waypoint in path_msg.poses:
        if(path_msg == hist_path):
            break
        # pose, angle = printlocation()
...
if __name__ == '__main__':
    ...
    try:
        ...
        #     if(path_msg != hist_path):
# UnboundLocalError: local variable 'hist_path' referenced before assignment