...
from spot_ros2.webrtc_client import WebRTCClient

from aiortc.rtp import RtcpRtpfbPacket
...
class InterceptStdErr:
    ...
    def write(self, data):
        ...
        #STDERR.error(data)
"""
added force_pli_routine()
Date: 03/18/2026
Function: 
Force Spot CAM to send I-frame by sending an RTCP PLI (Picture Loss Indication) packet every 2 seconds. 
This is necessary because the Spot CAM may not send I-frames frequently enough on its own, which lead to long delays and blurry video. 
By sending a PLI packet, we request the Spot CAM to send an I-frame immediately.
"""
async def force_pli_routine(client):
    while asyncio.get_event_loop().is_running():
        await asyncio.sleep(2.0)
        try:
            for receiver in client.getReceivers():
                if receiver.track.kind == "video":
                    
                    # acquire and make sure decoder source ssrc for the video track
                    media_ssrc = getattr(receiver, "_ssrc", 0)
                    if media_ssrc == 0 and hasattr(receiver, "_track"):
                        media_ssrc = getattr(receiver._track, "ssrc", 0)

                        packet = RtcpRtpfbPacket(
                            fmt=1,  # PLI
                            ssrc=12345, # dummy ssrc for the sender
                            media_ssrc=media_ssrc
                        )

                        if hasattr(receiver, "_transport"):
                            await receiver._transport._send_rtcp([packet])
                            logging.info(f"[RTCP] Sent PLI packet for media SSRC {media_ssrc}")
...
def start_webrtc(shutdown_flag, hostname, token, process_func, recorder=None):
    ...
    logging.info("-------------------------------------------------------------Starting WebRTC client...")
    print("-------------------------------------------------------------Starting WebRTC client...")

    asyncio.gather(
        client.start(), 
        process_func(client, shutdown_flag),
        monitor_shutdown(shutdown_flag, client),
        force_pli_routine(client) # start the PLI-force-send routine to ensure we get I-frames regularly
        )
    loop.run_forever()
...
async def monitorT(client, shutdown_flag, save=False):
    ...
    while asyncio.get_event_loop().is_running():
        ...
        try:
            ...
            def fast_convert(f):
                return f.to_ndarray(format="rgb24"), f.to_ndarray(format="bgr24")
            rgb_arr, bgr_arr = await loop.run_in_executor(None, fast_convert, frame)
            rgbImage = rgb_arr.copy()
            cvImage = bgr_arr.copy()
            cv2.imshow(w_name, cvImage)
            if save:
                cv2.imwrite('images/' + '{0:06d}'.format(frameCount) + '.jpg', cvImage)
            frameCount += 1
            c = cv2.waitKey(1)
            if c == 27:
                break
...
'''
03/18/2026
added fast_convert() and corresponding changes in captureT() to utilize C library for faster frame processing.
'''
# Frame processing occurs; otherwise it waits.
