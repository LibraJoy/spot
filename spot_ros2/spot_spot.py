...
def _connect():
    ...
    # # ADDED BY CLAUDE
    # # ========== CONFIGURE WEBRTC STREAM QUALITY ==========
    # try:
    #     from bosdyn.client.spot_cam.streamquality import StreamQualityClient
    #     from bosdyn.api.spot_cam import streamquality_pb2

    #     robot.logger.info("Configuring WebRTC stream quality...")
    #     stream_client = robot.ensure_client(StreamQualityClient.default_service_name)

    #     # Get current parameters
    #     current_params = stream_client.get_stream_params()
    #     current_bitrate = current_params.targetbitrate.value if hasattr(current_params.targetbitrate, 'value') else current_params.targetbitrate
    #     robot.logger.info(f"Current stream bitrate: {current_bitrate} bps ({current_bitrate/1e6:.1f} Mbps)")

    #     # Set higher bitrate for pano_full (5 cameras)
    #     # NOTE: Protobuf fields use .value for wrapped types
    #     params = streamquality_pb2.StreamParams()
    #     params.targetbitrate.value = 8000000  # 8 Mbps (for 5 cameras @ ~1.6 Mbps each)
    #     params.refreshinterval.value = 1      # Refresh every second
    #     params.idrinterval.value = 30         # IDR frame every 30 frames

    #     # Apply new settings
    #     stream_client.set_stream_params(params)
    #     robot.logger.info(f"Stream bitrate set to: 8.0 Mbps")

    #     # Enable congestion control for network adaptation
    #     stream_client.enable_congestion_control(True)
    #     robot.logger.info("Congestion control enabled")

    # except Exception as e:
    #     robot.logger.warning(f"Could not configure stream quality: {e}")
    #     robot.logger.warning("Continuing with default stream settings...")
    # # ===================================================== 
    # # /ADDED BY CLAUDE
...
def webrtc_frame_count():
    return spot_webrtc.frame_count
